...
var reservedWords, nativeClasses, globalFunctions, labels, desc, udata, dictionary = {};
...
function sorter(e1, e2, e, prefix) {
	...
	if (!e1c && e2c) r = 1;
		...
		ei = e1c.index - e2c.index;
		ei1 = e1c[0].indexOf(prefix) === 0;
		ei2 = e2c[0].indexOf(prefix) === 0;
		...
		var words = !plc ? sugg : sugg.filter((s) => s.toLowerCase().match(e)),
			msugg = words.map((w) => {
				return {
					...
					leftLabelHTML: udata.wordLeftLabel || "ProtoSS",
					type: "w"
				}
			});
		var words2 = !plc ? sugg2 : sugg2.filter((s) => s.toLowerCase().match(e)),
			msugg2 = words2.map((w) => {
				return {
					...
					leftLabelHTML: udata.classLeftLabel || "ProtoSS",
					type: "class"
				}
			});
		var words3 = !plc ? sugg3 : sugg3.filter((s) => s.toLowerCase().match(e)),
			msugg3 = words3.map((w) => {
				return {
					...
					leftLabelHTML: udata.funcLeftLabel || "ProtoSS",
					type: "function"
				}
			});
		var cwords = msugg.concat(msugg2).concat(msugg3);
		cwords.sort((e1, e2) => sorter(e1, e2, e, prefix));
		return cwords;
	}
}
...
class AsyncProvider {
	...
	filterData(prefix, sugg) {
		...
		words.sort((e1, e2) => sorter(e1, e2, e, prefix));
		return words;
	}
