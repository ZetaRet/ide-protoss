...
function declareTS(desc, joiner, tab, pipe) {
	...
	props.forEach(e => {
		...
		for (k in e) {
			...
			propsts.push(tab.repeat(tabrep + 1) + k + ': ' + p.map.map(pt => TSConvert[pt] || pt).join(pipe) + ';');
		}
	});
	sprops.forEach(e => {
		var k, p;
		for (k in e) {
			p = utils.parsePropertyType(e[k]);
			spropsts.push(tab.repeat(tabrep + 1) + k + ': ' + p.map.map(pt => TSConvert[pt] || pt).join(pipe) + ';');
		}
	});
	methods.forEach(e => {
		var k, m, arg, ret;
		for (k in e) {
			...
			arg = m.args.map.map(a => (a[0].endsWith('*') ? a[0].substring(0, a[0].length - 1) : a[0] + '?') + ': ' + a[1].map(t => TSConvert[t] || t).join(pipe)).join(', ');
			ret = m.ret.map.map(r => TSConvert[r] || r.replace('::', '.')).join(pipe);
