...
var cfg = {
	...
	cfgkeys = [
		...
		'styleGroupMark', 'breadcrumbsOptions'
	],
...
var titleEl, fileEl, linesEl, tokensEl, tokensElTooltip, sepEl, packEl, jsonEl, analyEl, packagepanel, topEl, dynamicCSS, markers, cpicon;
...
function createPathTooltip(el, ct) {
	...
	readers.addTooltip(el, ['Browse:<br/><br/><b>' + ct + '</b><br/>---<br/><div class="protoss-path-tooltip"></div>'], tooltipst, null, (tip, e) => {
		if (e.shiftKey) electron.shell.showItemInFolder(ct);
		else {
			tip.hideOnClickOutsideOfTooltip = function(he) {
				var t = tip.getTooltipElement();
				if (t !== he.target && !t.contains(he.target)) tip.hide();
			};
			tip.show();
			var browser = tip.getTooltipElement().getElementsByClassName('protoss-path-tooltip')[0];
			if (atom.project.getPaths().find(pp => ct.indexOf(pp) === 0)) {
				try {
					browser.innerHTML = fs.readdirSync(ct).map(e => '<span class="protoss-tooltip-span">' + e + '</span>').join('<br/>');
					[].slice.call(browser.getElementsByTagName('span')).forEach(s => {
						s.style.cursor = 'pointer';
						s.addEventListener('click', se => {
							var sepa = ct + path.sep + s.innerText;
							if (fs.statSync(sepa).isFile()) {
								atom.open({
									pathsToOpen: [sepa],
									newWindow: false
								});
...
function initUI() {
	[titleEl, fileEl, linesEl, tokensEl, tokensElTooltip, sepEl, packEl, jsonEl, analyEl] = ui.initUI(
