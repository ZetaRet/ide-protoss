...
function updateFromDescFile(descjson, nopush) {
	...
	if (descjson) {
		...
		if (impspan && jimp.length > 0) {
			impspan.style.cursor = 'pointer';
			atom.tooltips.add(impspan, {
				title: '<div style="' + tooltipst + '">' + jimp.join('<br/>') + '</div>',
				trigger: 'click'
			});
			impspan.setAttribute('title', 'Click to view Class Imports');
			impspan.classList.add('protoss-ihover');
		}
