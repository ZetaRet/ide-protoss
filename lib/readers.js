...
var parsePropertyType, parseReturnType, parseArgumentsTypes, getFileExt, pushNotification, utils;
...
function setUtils(pt, rt, at, fe, pn, ut) {
...
function addTooltip(span, props, tooltipst, title, onclick, trigger, cls, tooltipcls, maincls) {
	if (props && props.constructor !== Array) props = [props];
...
function updateFromDescFile(descjson, nopush) {
	...
	if (descjson) {
		...
		obtainDescData(descjson, null, adhocInnerTypes, [jprop, jmeth, '', counter, 'protoss-tooltip-row']);
		pc = counter.pc;
		spc = counter.spc;
		mc = counter.mc;
		smc = counter.smc;
		...
		if (descjson.imports && descjson.imports.length > 0) {
			descjson.imports.forEach(e => jimp.push('<div class="protoss-tooltip-row icon icon-package">' + e + '</div>'));
		...
		addTooltip(impspan, jimp.join(''), tooltipst, 'Click to view Class Imports', (t, e) => {
			var tel = t.getTooltipElement();
			[].slice.call(tel.getElementsByClassName('protoss-tooltip-row')).forEach(s => {
		...
		addTooltip(propspan, jprop.join(''), tooltipst, 'Click to view Properties Tooltip');
		addTooltip(methspan, jmeth.join(''), tooltipst, 'Click to view Methods Tooltip');
...
function readInheritance(descjson, callback, key, descKey, loader) {
	var timeoutMS = main.cfg.asyncOptions.inheritLoad || 5;
	if (!key) key = "inhTree";
...
function adhocInnerTypes(rd, descjson, k, v, d, e, m, a, at, i, adata) {
	var attrStr, attr = [],
		...
		counter = adata[3],
		cls = adata[4];
	if (tst) attr.push('style="' + tst + '"');
	if (cls) attr.push('class="' + cls + '"');
	attrStr = attr.join(' ');
	if (a === 'property') {
		...
		try {
			jprop.push('<div ' + attrStr + '><b>' + k + '</b> - ' + utils.convertHTMLTypes(parsePropertyType(v).text) + '</div>');
		} catch (er) {
		...
		try {
			cmt = utils.convertMethodTypes(k, v);
			jmeth.push('<div ' + attrStr + '><b>' + k + '</b>(' + utils.convertHTMLTypes(cmt.argstext) + ') : ' + utils.convertHTMLTypes(cmt.rettext) + '</div>');
		} catch (er) {
			if (main.cfg.console === 'yes') console.log(er, a, e, k, v);
		}
	}
...
function adhocDescTypes(rd, descjson, k, v, d, e, m, a, at, i, adata) {
	try {
		if (a === 'property') {
			...
			var dspl, ret, cmt = utils.convertMethodTypes(k, v);
			ret =cmt.ret.text;
			dspl = cmt.args.spl;
			if (ret || dspl.length > 0) {
				if (!descjson.imports) descjson.imports = [];
				cmt.args.map.forEach(ee => {
					ee[1].forEach(e => {
						if (e && descjson.imports.indexOf(e) === -1) descjson.imports.push(e);
					});
				});
				if (ret) {
					cmt.ret.map.forEach(e => {
						if (e && descjson.imports.indexOf(e) === -1) descjson.imports.push(e);
					});
				}
			}
...
function obtainDescAutoComplete(descjson) {
	...
	if (descjson) {
		var desc = [];
		if (descjson.description) desc.push(descjson.description.split('`').join(''));
		if (descjson.text) desc.push(descjson.text.split('`').join(''));
