...
function readDescFile(e, b) {
	...
	if (descjson) {
		...
		inh.forEach(function(el) {
			var sn, samens, fn, e;
			var rpn = getResolvedPathName(el, getFileExt(), false, p);
			samens = rpn[0];
			fn = rpn[1];
			e = rpn[3];
			...
			if (samens) {
				...
				ele.addEventListener('click', function(e) {
					if (e.ctrlKey) {
						electron.shell.showItemInFolder(fnp);
					} else {
						atom.open({
							pathsToOpen: [fnp],
							newWindow: false
...
function getResolvedPathName(el, ext, asdox, p) {
	var jpath = getJpath();
...
function getInheritance(descjson, key) {
	...
	var inh = descjson[key] && descjson[key].constructor === Array ? descjson[key] : [],
		newinh = [];
	inh.forEach(function(el) {
		newinh.push(getResolvedPathName(el, 'json', true));
	});
...
function readProjectDox(files, cdata) {
	...
	main.cfg.atomprojsons = [];
	files.forEach(function(f) {
		var fpath = f[0],
			file = f[1],
			fd = fpath + path.sep + file;
		...
		if (fs.existsSync(fd)) {
			fs.readFile(fd, function(e, b) {
				...
				loaded--;
				main.cfg.atomprojsons.push(j);
				if (cdata.onread) cdata.onread(j, f);
				if (loaded === 0 && cdata.loaded) cdata.loaded(files);
			});
		}
	});
}
