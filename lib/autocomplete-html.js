...
var extracts = ['EventTarget', 'Node', 'Element',
	...
	'HTMLImageElement', 'HTMLInputElement', 'HTMLButtonElement', 'HTMLTextAreaElement', 'HTMLSelectElement',
	...
	'Event', 'ErrorEvent', 'History', 'ImageData', 'Navigator', 'Location',
...
class DescSymCls extends String {
	constructor() {
		super('HTMLElements can be extended by new class.\nUse customElements.define & document.createElement("localName") to create one.');
	}
}
...
class Extractor {
	...
	static getFromPrototype(ele) {
		...
		Extractor.getProtochain(ele.prototype, ex);
...
extracts.forEach((e, i, a) => {
	if (window[e]) {
		var pchainprefix, pchainname, extra = Extractor.getFromPrototype(window[e]);
		[pchainprefix, pchainname] = Extractor.getProtoDesc(extra);
