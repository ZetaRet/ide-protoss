...
function readDescFile(e, b) {
	...
	if (descjson) {
		...
		var inh = descjson.inherits,
			newinh = [];
		atxt = 'Properties: ' + pc + '-' + spc + ', Methods: ' + mc + '-' + smc + ', Inherits: ';
...
function updateProtoSSCrumbs(editor) {
	...
	if (jsfile) {
		TokenUtil.refreshTokens(editor);
		...
		if (md && mp) {
			...
			atom.workspace.open(p).then(function(editor) {
				...
				if (!t) {
					...
					jdesc['supername'] = snsplit.length === 2 && !snsplit[0] ? snsplit[1] : sn;
...
class TokenUtil {
	...
	static getTabLength() {
		var tabLength = atom.config.settings.editor.tabLength || atom.config.defaultSettings.editor.tabLength;
		return tabLength;
	}

	static getSpaces(length) {
		var s = '';
		while (length--) s += ' ';
		return s;
	}
	...
	static ctrlClick() {
		...
		try {
			...
			if (ct) {
				...
				if (prev && prev.length > 0) cfg.editor.setCursorBufferPosition([prev[0].row, prev[0].start]);
			}
		} catch (e) {
			cfg.editor.setTabLength(tl);
			throw (e);
	...
	static click() {
		...
		try {
			...
			if (ct) {
				...
				if (next && next.length > 0) cfg.editor.setCursorBufferPosition([next[0].row, next[0].start]);
			}
		} catch (e) {
			cfg.editor.setTabLength(tl);
			throw (e);
