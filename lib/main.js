...
var cfg = {},
    nskey = 'ide-protoss.',
    cfgkeys = ['breadcrumbs', 'console', 'textColor'],
    cfgobserve = {};
...
function readDocJson(e, b) {
    ...
    if (projson && projson.type === 'protossdox') {
        ...
        jpath.pop();
        jpath = jpath.join('\\') + '\\';
        var cpath = p.replace(jpath, '').split('.');
        cpath.pop();
        cpath = cpath.join('.');
...
function readDescFile(e, b) {
    ...
    if (descjson) {
        ...
        var inh = descjson.inherits,
            newinh = [];
        atxt = 'Properties: ' + pc + ', Methods: ' + mc + ', Inherits: ';
...
function updateProtoSSCrumbs(editor) {
    ...
    if (jsfile) {
        cfg.activeTokens = gjs.tokenizeLines(editor.getText());
...
var tokenArr = gjs.tokenizeLines(testTokens);
