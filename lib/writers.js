...
function declareTS(desc, joiner, tab, pipe) {
	...
	props.forEach((e, i) => {
		var k, p, a = (cfg.exportOptions.accessor === 'yes' ? TSAccess[i] : '');
		for (k in e) {
			p = utils.parsePropertyType(e[k]);
			propsts.push(tab.repeat(tabrep + 1) + a + k + ': ' + getPropertyTypes(p).join(pipe) + ';');
		}
	});
	sprops.forEach(e => {
		var k, p;
	...
	methods.forEach((e, i) => {
		var k, m, arg, ret, a = (cfg.exportOptions.accessor === 'yes' ? TSAccess[i] : '');
		for (k in e) {
			...
			else methodsts.push(tab.repeat(tabrep + 1) + a + k + '(' + arg + '): ' + (ret || 'void'));
		}
	});
	smethods.forEach(e => {
		var k, m, arg, ret;
