...
function initUI(topEl, c, TokenUtil, ProtoSSUtil, updateProtoSSCrumbs, gd, formatBytes, readers, xutils) {
	...
	packEl.addEventListener('mouseover', function(e) {
		var noap, ap, namesp = cfg.projson ? cfg.projson.namespace : '',
			version = cfg.projson ? cfg.projson.version : '',
			license = cfg.projson ? cfg.projson.license : '',
			at = 'Access ' + cfg.docjsonname + ' [CTRL+SHIFT+ALT+F7]';
		if (namesp) at += '\nNamespace: ' + namesp;
		if (version) at += '\nVersion: ' + version;
		if (license) at += '\nLicense: ' + license;
