...
reservedWords = reservedWords.map(e => new Word(e));
...
class VType2Cls extends String {
	constructor() {
		super('variable');
	}
}
...
var nativeClasses = [],
	...
	vtype2Sym = new VType2Cls(),
	leftSym = new LeftSymCls();

reservedWords.forEach(e => {
	labels.set(e, labelSym);
	desc.set(e, descSym);
	vtype.set(e, vtypeSym);
	lefts.set(e, leftSym);
});

var wordValuesFontWeight = ['normal', 'bold', 'lighter', 'bolder'],
	fontWeightSym = new DescVSymCls(['font-weight']);
wordValuesFontWeight = wordValuesFontWeight.map(e => new Word(e));
wordValuesFontWeight.forEach(e => {
	reservedWords.push(e);
	labels.set(e, labelVSym);
	desc.set(e, fontWeightSym);
	vtype.set(e, vtype2Sym);
	lefts.set(e, leftSym);
});

var wordValuesPointerEvents = ['auto', 'none', 'visiblePainted', 'visibleFill', 'visibleStroke', 'visible', 'painted', 'fill', 'stroke', 'all'],
	pointerEventsSym = new DescVSymCls(['pointer-events']);
wordValuesPointerEvents = wordValuesPointerEvents.map(e => new Word(e));
wordValuesPointerEvents.forEach(e => {
	reservedWords.push(e);
	labels.set(e, labelVSym);
	desc.set(e, pointerEventsSym);
	vtype.set(e, vtype2Sym);
	lefts.set(e, leftSym);
});

var wordValuesCursor = ['auto', 'none', 'default', 'context-menu', 'help', 'pointer', 'progress', 'cell', 'crosshair', 'text', 'vertical-text',
	...
	cursorSym = new DescVSymCls(['cursor']);
wordValuesCursor = wordValuesCursor.map(e => new Word(e));
wordValuesCursor.forEach(e => {
	reservedWords.push(e);
	labels.set(e, labelVSym);
	desc.set(e, cursorSym);
	vtype.set(e, vtype2Sym);
	lefts.set(e, leftSym);
});

var wordValuesDisplay = ['block', 'inline', 'run-in', 'none', 'contents', 'inline-block', 'inline-table', 'inline-flex', 'inline-grid',
	...
	displaySym = new DescVSymCls(['display']);
wordValuesDisplay = wordValuesDisplay.map(e => new Word(e));
wordValuesDisplay.forEach(e => {
	reservedWords.push(e);
	labels.set(e, labelVSym);
	desc.set(e, displaySym);
	vtype.set(e, vtype2Sym);
	lefts.set(e, leftSym);
});
