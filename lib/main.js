...
var titleEl, fileEl, linesEl, tokensEl, tokensElTooltip, sepEl, packEl, jsonEl, analyEl, packagepanel, topEl, dynamicCSS, cursors;
...
function updateProtoSSCrumbs(editor, wait) {
	...
	if (jsfile) {
		...
		editor.disposeCursor = editor.onDidChangeCursorPosition((c, nbp, nsp, obp, osp, tc) => {
			if (cursors) cursors.forEach(e => e ? e.destroy() : null);
			cursors = MarkerUtil.decorate(nbp, true);
		});
