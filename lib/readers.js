...
function addTooltip(span, props, tooltipst, title, onclick) {
	if (span && props.length > 0) {
		span.style.cursor = 'pointer';
		atom.tooltips.add(span, {
			title: '<div style="' + tooltipst + '">' + props.join('<br/>') + '</div>',
			trigger: 'click'
		});
		span.setAttribute('title', title);
		span.classList.add('protoss-ihover');
		if (onclick) span.addEventListener('click', e => onclick(atom.tooltips.findTooltips(span)[0]));
	}
}
...
function updateFromDescFile(descjson, nopush) {
	...
	if (descjson) {
		...
		var tooltipst = ['text-align: left !important',
			'max-width:' + Math.round(window.innerWidth / 2) + 'px',
			'overflow:overlay',
			'padding:5px 20px',
			'max-height:' + Math.round(window.innerHeight / 2) + 'px'
		].join(';') + ';';
		addTooltip(impspan, jimp, tooltipst, 'Click to view Class Imports', t => {
