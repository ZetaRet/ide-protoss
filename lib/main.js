	...
	if (descjson) {
		...
		if (descjson.public_properties) {
			for (k in descjson.public_properties) {
				pc++;
				jprop.push('<div style="' + tst + '"><b>' + k + '</b> - ' + descjson.public_properties[k] + '</div>');
			}
		...
		if (descjson.public_methods) {
			for (k in descjson.public_methods) {
				...
				jmeth.push('<div style="' + tst + '"><b>' + k + '</b>(' + dspl[0] + ') : ' + (ret || 'void') + '</div>');
			}
		...
		if (descjson.static && descjson.static.public_properties) {
			...
			for (k in descjson.static.public_properties) {
				...
				jprop.push('<div style="' + tst + '"><b>' + k + '</b> - ' + descjson.static.public_properties[k] + '</div>');
			}
		}
		...
		var inh = descjson.inherits && descjson.inherits.constructor === Array ? descjson.inherits : [],
			newinh = [];
		atxt = '<span class="json-prop" style="cursor:pointer">Properties: ' + pc + '-' + spc + '</span>,' +
			' <span class="json-meth" style="cursor:pointer">Methods: ' + mc + '-' + smc +
			'</span>, Inherits: ';
		var ft = [];
