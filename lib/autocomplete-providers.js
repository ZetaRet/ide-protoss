...
class CommonProvider {
	...
	fuzzyReg(plc) {
		return new RegExp('[' + plc.split('').join('][\\w|\\W]*[') + '][\\w|\\W]*');
	}

	sorthem(plc, e, words) {
		if (words.length > 0 && main.cfg.autoCompleteSort === 'yes' && plc && plc.charAt(0).match(new RegExp('\\w'))) words.sort((e1, e2) => sorter(e1, e2, e, plc));
	}
}
...
class MergerProvider extends CommonProvider {
	...
	getSuggestions(options) {
		...
		var plc = options.prefix.toLowerCase(),
			words = this.words,
			e = this.fuzzyReg(plc);
		this.words = [];
		this.sorthem(plc, e, words);
