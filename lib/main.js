...
function initBindings() {
	...
	bindings[cfg.packageid + ':cmdts'] = function() {
		navigator.clipboard.writeText((cfg.descjson ? writers.declareTS(cfg.descjson, '\n', '\t') : '') || 'declare namespace no.json.descriptor {}');
	};
...
function exeFileCommand() {
	if (!cfg.exec) cfg.exec = require('child_process').exec;

	function Callback(err, stdout, stderr) {
		if (err) {
			if (cfg.console === 'yes') console.log(`exec error: ${err}`);
			return;
		} else {
	...
	var res = cfg.exec(cfg.exeOptions.nodeExe + ' ' + atom.workspace.getActiveTextEditor().getPath(), Callback);
	if (cfg.console === 'yes') console.log(res, res.pid);
}
...
function prefixInit() {
	...
	ficon.innerHTML = '<i class="icon icon-terminal" style="padding-right: 25px">IDE</i>';
	...
	cfg.emitter.on('protoss_nullElements', () => tokenStatus.innerText = '');
