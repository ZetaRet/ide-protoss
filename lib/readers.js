...
function getInheritance(descjson) {
	var inh = descjson.inherits && descjson.inherits.constructor === Array ? descjson.inherits : [],
		newinh = [];
	var jpath = getJpath();
	inh.forEach(function(el) {
		...
		var inhp;
		inhp = doxrepl;
		fn = (inhp + fn + '.json').split('/').join('\\');

		newinh.push([samens, fn]);
	});

	return newinh;
}

function readInheritance(descjson, callback) {
	if (!descjson.inhTree) {
		descjson.inhTree = getInheritance(descjson);
		if (descjson.inhTree && descjson.inhTree.length > 0) {
			setTimeout(function() {
				var i, f, fd;
				for (i = 0; i < descjson.inhTree.length; i++) {
					f = descjson.inhTree[i];
					if (!f[0]) continue;
