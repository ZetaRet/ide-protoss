...
var cfg = {
	...
	nskey = cfg.packageid + '.',
	cfgkeys = ['autoUpdateOnChange', 'breadcrumbs', 'console',
...
function readDocJson(e, b) {
	...
	if (projson && projson.type === 'protossdox') {
		...
		var srcrepl = srcpath.split('/').join('\\');
		var cpath = p.indexOf(srcrepl) >= 0 ? p.replace(srcrepl, '').split('.') : p.replace(jpath, '').split('.');
		var cext = cpath.length > 1 ? cpath.pop() : '';
		cpath = cpath.join('.');
...
function readDescFile(e, b) {
	...
	if (descjson) {
		...
		inh.forEach(function(el) {
			...
			var fext = getFileExt();
			fn = (jpath + (cfg.projson.src || '') + fn + (fext ? '.' + fext : '')).split('/').join('\\');
	...
	static refreshTokens(editor, asyncit, nonot) {
		...
		if (!asyncit) TokenUtil.refreshTokensDefault(editor);
			...
			TokenUtil.tokenizeLinesAsync(editor, function(tokens) {
				...
				if (!nonot && cfg.notifications === 'yes') atom.notifications.addInfo((cfg.groupdigits === 'yes' ? groupDigits(nt) : nt) + ' tokens loaded in ' + editor.getPath());
			});
		...
		try {
			...
			if (ct) {
				...
				if (prev && prev.length > 0) cfg.editor.setCursorBufferPosition([prev[0].row, prev[0].start + prev[0].value.indexOf(prev[0].value.trim())]);
					...
					if (prev && prev.length > 0 && prev[0] !== ct) cfg.editor.setCursorBufferPosition([prev[0].row, prev[0].start + prev[0].value.indexOf(prev[0].value.trim())]);
				}
			}
		} catch (e) {
			cfg.editor.setTabLength(tl);
		...
		try {
			...
			if (ct) {
				...
				if (next && next.length > 0) cfg.editor.setCursorBufferPosition([next[0].row, next[0].start + next[0].value.indexOf(next[0].value.trim())]);
					...
					if (next && next.length > 0 && next[0] !== ct) cfg.editor.setCursorBufferPosition([next[0].row, next[0].start + next[0].value.indexOf(next[0].value.trim())]);
				}
			}
		} catch (e) {
			cfg.editor.setTabLength(tl);
		...
		if (backwards) {
			if (i !== -1) {
				...
				for (; i >= 0; i--) {
					st = line[i];
					tv = st.value;
					if (cfg.findTokenCaseSensitivity === 'yes' ? tv.trim() === trimv : tv.trim().toLowerCase() === trimvl) {
						st.end = cc;
...
tokensEl.style.pointerEvents = 'none';
var tokensElTooltip = ['Click to next token', 'CTRL + Click to previous token', 'SHIFT + Click to select token', 'ALT + Click to refresh tokens'];
