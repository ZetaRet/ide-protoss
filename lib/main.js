...
function updateProtoSSCrumbs(editor, wait) {
	...
	if (jsfile) {
		cfg.altered = true;
		if (!wait || !cfg.tokenTimeout) TokenUtil.refreshTokens(editor);
		else {
			tokensEl.innerHTML = 'wait tokens';
			cfg.tokenTimeoutId = setTimeout(function() {
				if (editor === atom.workspace.getActiveTextEditor()) TokenUtil.refreshTokens(editor);
				else {
...
atom.packages.onDidActivatePackage(function(package){
	if(package.name === 'ide-protoss'){
		package.TokenUtil = TokenUtil;
		package.ExternalUtil = ExternalUtil;
	}
});
var ideprotoss_pack = atom.packages.getActivePackage('ide-protoss');
if (ideprotoss_pack) {
...
bindings['ide-protoss:supers'] = function() {
