	...
	if (editors && editors.length > 0) {
		...
		words = editors.map(te => {
			var ten = te.getFileName(),
				setw = te.__bufferWords || Array.from(new Set(te.buffer.getText().split(getWordSplitter()))).filter(w => w && w.length >= wl);
			setw.forEach(w => {
				if (!desc['#' + w]) desc['#' + w] = [];
				desc['#' + w].push(ten);
			});
...
class MergerProvider extends CommonProvider {
	...
	getPrefix(options) {
		...
		if (main.cfg.autoCompleteOptions.mergerProvider.indexOf('Word') !== -1)
			prefix = this.getWordPrefix(options.prefix, options.editor, options.bufferPosition);
		return prefix;
	}
