...
function obtainDescAutoComplete(descjson) {
	...
	if (descjson.public_properties) {
		for (k in descjson.public_properties) {
			v = descjson.public_properties[k];
			d = descjson.descriptions[k];
			rd.push({
				text: k,
				description: v + "\n" + (d || ""),
				rightLabelHTML: "Public property",
				leftLabelHTML: "ProtoSS",
				type: "property"
			});
		}
	}
	if (descjson.private_properties) {
		for (k in descjson.private_properties) {
			v = descjson.private_properties[k];
			d = descjson.descriptions[k];
			rd.push({
				text: k,
				description: v + "\n" + (d || ""),
				rightLabelHTML: "Private property",
				leftLabelHTML: "ProtoSS",
				type: "property"
			});
		}
	...
	if (descjson.protected_properties) {
		for (k in descjson.protected_properties) {
			v = descjson.protected_properties[k];
			d = descjson.descriptions[k];
			rd.push({
				text: k,
				description: v + "\n" + (d || ""),
				rightLabelHTML: "Protected property",
				leftLabelHTML: "ProtoSS",
				type: "property"
			});
		}
	}
	if (descjson.static.public_properties) {
		for (k in descjson.static.public_properties) {
			v = descjson.static.public_properties[k];
			d = descjson.static_descriptions[k];
			rd.push({
				text: k,
				description: v + "\n" + (d || ""),
				rightLabelHTML: "Public static property",
				leftLabelHTML: "ProtoSS",
				type: "property"
			});
		}
	}
	if (descjson.static.private_properties) {
		for (k in descjson.static.private_properties) {
			v = descjson.static.private_properties[k];
			d = descjson.static_descriptions[k];
			rd.push({
				text: k,
				description: v + "\n" + (d || ""),
				rightLabelHTML: "Private static property",
				leftLabelHTML: "ProtoSS",
				type: "property"
			});
		}
	}
	if (descjson.static.protected_properties) {
		for (k in descjson.static.protected_properties) {
			v = descjson.static.protected_properties[k];
			d = descjson.static_descriptions[k];
			rd.push({
				text: k,
				description: v + "\n" + (d || ""),
				rightLabelHTML: "Protected static property",
				leftLabelHTML: "ProtoSS",
				type: "property"
			});
		}
	...
	if (descjson.public_methods) {
		for (k in descjson.public_methods) {
			v = descjson.public_methods[k];
			d = descjson.descriptions[k];
			md = getMethodDescAutoComplete(d);
			rd.push({
				text: k,
				description: v + "\n" + md.join("\n"),
				rightLabelHTML: "Public method",
				leftLabelHTML: "ProtoSS",
				type: "method"
			});
		}
	}
	if (descjson.private_methods) {
		for (k in descjson.private_methods) {
			v = descjson.private_methods[k];
			d = descjson.descriptions[k];
			md = getMethodDescAutoComplete(d);
			rd.push({
				text: k,
				description: v + "\n" + md.join("\n"),
				rightLabelHTML: "Private method",
				leftLabelHTML: "ProtoSS",
				type: "method"
			});
		}
	...
	if (descjson.protected_methods) {
		for (k in descjson.protected_methods) {
			v = descjson.protected_methods[k];
			d = descjson.descriptions[k];
			md = getMethodDescAutoComplete(d);
			rd.push({
				text: k,
				description: v + "\n" + md.join("\n"),
				rightLabelHTML: "Protected method",
				leftLabelHTML: "ProtoSS",
				type: "method"
			});
		}
	}
	if (descjson.static.public_methods) {
		for (k in descjson.static.public_methods) {
			v = descjson.static.public_methods[k];
			d = descjson.static_descriptions[k];
			md = getMethodDescAutoComplete(d);
			rd.push({
				text: k,
				description: v + "\n" + md.join("\n"),
				rightLabelHTML: "Public static method",
				leftLabelHTML: "ProtoSS",
				type: "method"
			});
		}
	}
	if (descjson.static.private_methods) {
		for (k in descjson.static.private_methods) {
			v = descjson.static.private_methods[k];
			d = descjson.static_descriptions[k];
			md = getMethodDescAutoComplete(d);
			rd.push({
				text: k,
				description: v + "\n" + md.join("\n"),
				rightLabelHTML: "Private static method",
				leftLabelHTML: "ProtoSS",
				type: "method"
			});
		}
	}
	if (descjson.static.protected_methods) {
		for (k in descjson.static.protected_methods) {
			v = descjson.static.protected_methods[k];
			d = descjson.static_descriptions[k];
			md = getMethodDescAutoComplete(d);
			rd.push({
				text: k,
				description: v + "\n" + md.join("\n"),
				rightLabelHTML: "Protected static method",
				leftLabelHTML: "ProtoSS",
				type: "method"
			});
		}
