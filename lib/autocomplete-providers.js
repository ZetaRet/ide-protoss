...
class AsyncProvider {
	...
	getSuggestions(options) {
		if (main.cfg.autoComplete === 'no' || udata.types.indexOf(main.cfg.activeGrammar.name) === -1 || udata.noAsync) return [];
		return this.findMatchingSuggestions(options.prefix, options);
	}

	asyncData(resolve, prefix, options) {
		if (this.fetchAsyncData) this.fetchAsyncData(resolve, prefix, options);
		else resolve([]);
	}
