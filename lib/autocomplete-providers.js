...
var gjs, reservedWords, nativeClasses, globalFunctions, labels, desc, udata, lefts, vtype, dictionary = {};
...
function setGrammar(xgjs) {
	gjs = xgjs;
	updateData(data);
	if (main.cfg.autoCompleteCSS === 'yes') {
		loadDictionary('css', './autocomplete-css.js');
		updateData(getMerged(['css'], true, gjs.name));
	}
}
...
function getMerged(ids, includeData, gid) {
	...
	if (includeData && (!gid || data.types.indexOf(gid) !== -1)) {
		...
		for (k in labels) m.labels[k] = labels[k];
		for (k in desc) m.desc[k] = desc[k];
		for (k in lefts) m.lefts[k] = lefts[k];
		for (k in vtype) m.vtype[k] = vtype[k];
	...
	for (i = 0; i < ids.length; i++) {
		...
		if (!gid || d.types.indexOf(gid) !== -1) {
			...
			for (k in d.labels) m.labels[k] = d.labels[k];
			for (k in d.desc) m.desc[k] = d.desc[k];
			for (k in d.lefts) m.lefts[k] = d.lefts[k];
			for (k in d.vtype) m.vtype[k] = d.vtype[k];
		}
	...
	findMatchingSuggestions(prefix) {
		...
		var words = !plc ? sugg : sugg.filter((s) => s.toLowerCase().match(e)),
			msugg = words.map((w) => {
				return {
					text: w,
					description: desc[w] || udata.wordDesc || "Lexical grammar",
					rightLabelHTML: labels[w] || udata.wordRightLabel || "Reserved word",
					leftLabelHTML: lefts[w] || udata.wordLeftLabel || "ProtoSS",
					type: (vtype ? vtype[w] : null) || "w"
				}
			});
		var words2 = !plc ? sugg2 : sugg2.filter((s) => s.toLowerCase().match(e)),
			msugg2 = words2.map((w) => {
				return {
					text: w,
					description: desc[w] || udata.classDesc || "Standard built-in objects",
					rightLabelHTML: labels[w] || udata.classRightLabel || "Native Classes",
					leftLabelHTML: lefts[w] || udata.classLeftLabel || "ProtoSS",
					type: (vtype ? vtype[w] : null) || "class"
				}
			});
		var words3 = !plc ? sugg3 : sugg3.filter((s) => s.toLowerCase().match(e)),
			msugg3 = words3.map((w) => {
				return {
					text: w,
					description: desc[w] || udata.funcDesc || "Global functions",
					rightLabelHTML: labels[w] || udata.funcRightLabel || "Functions",
					leftLabelHTML: lefts[w] || udata.funcLeftLabel || "ProtoSS",
					type: (vtype ? vtype[w] : null) || "function"
				}
			});
		var cwords = msugg.concat(msugg2).concat(msugg3);
		if (prefix && prefix.charAt(0).match(new RegExp('\\w'))) cwords.sort((e1, e2) => sorter(e1, e2, e, prefix));
		return cwords;
	}
}
...
class AsyncProvider {
	...
	filterData(prefix, sugg) {
		...
		if (prefix && prefix.charAt(0).match(new RegExp('\\w'))) words.sort((e1, e2) => sorter(e1, e2, e, prefix));
		return words;
	}
