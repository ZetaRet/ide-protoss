	...
	static resolve(options) {
		...
		try {
			...
			if (ct) {
				...
				if (!res) {
					try {
						f = readers.getResolvedPathNameProject(ct.value, getFileExt(), false, ptr);
						if (f[0]) {
							rp = f[1].split('\\').join(path.sep);
							res = ProtoSSUtil.openFile(rp, options);
						}
						if (!res) {
							f = readers.getResolvedPathNameProject(ct.value, '', false, ptr);
							if (f[0]) {
								rp = f[1].split('\\').join(path.sep);
								res = ProtoSSUtil.openFile(rp, options);
							}
...
function initUI() {
	...
	packEl.addEventListener('mouseover', function(e) {
		var ap, t = cfg.projson ? cfg.projson.namespace : '',
			at = 'Access ' + cfg.docjsonname + ' [CTRL+SHIFT+ALT+F7]' + (t ? '\nNamespace: ' + t : '');
		if (cfg.atomprojsons && cfg.atomprojsons.length > 0) {
			ap = [];
			cfg.atomprojsons.forEach(e => e[0].bind !== false ? ap.push(e[0].name + (e[0].namespace ? ': ' + e[0].namespace : '')) : null);
			at += '\n-\n' + ap.join('\n');
		}
