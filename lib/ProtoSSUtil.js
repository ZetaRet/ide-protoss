...
class ProtoSSUtil {
	...
	static projectSpace() {
		...
		ps = {
			namesp, version, license, cpj, capj, ap, noap
		};
	...
	static cachedJSON(fd, complete, remove, beforeRead, afterRead, nocache) {
		var res = false;
		if (jsonCache[fd]) {
			if (remove) delete jsonCache[fd];
			complete(fd, jsonCache[fd], null);
		} else if (fs.existsSync(fd)) {
			if (beforeRead) beforeRead(fd);
			fs.readFile(fd, function(e, b) {
				...
				if (j && !nocache) jsonCache[fd] = j;
				res = true;
				complete(fd, j, e);
			});
			if (afterRead) afterRead(fd);
		}
		return res;
	}
	...
	static clearCache() {
		var key;
		for (key in jsonCache) delete jsonCache[key];
	}
}
