...
var titleEl, fileEl, linesEl, tokensEl, tokensElTooltip, sepEl, packEl, jsonEl, analyEl, packagepanel, topEl, dynamicCSS, markers;
...
function updateProtoSSCrumbs(editor, wait) {
	...
	if (jsfile) {
		...
		setTimeout(() => {
			editor.disposeChangeEach = editor.onDidChange(function(changeEvent) {
				if (changeEvent && changeEvent.constructor === Array) {
					editor.__changing = true;
					if (markers) markers.forEach(e => e ? e.destroy() : null);
					markers = null;
				}
			});
		}, 0);
		editor.disposeChange = editor.onDidStopChanging(function(changes) {
			editor.__changing = false;
			TokenUtil.Alter();
		});
		editor.disposeCursor = editor.onDidChangeCursorPosition((c, nbp, nsp, obp, osp, tc) => {
			if (!editor.__changing && !cfg.altered) {
				if (markers) markers.forEach(e => e ? e.destroy() : null);
				markers = cfg.styleGroupMark.markerAllow === 'yes' ? MarkerUtil.decorate(null, nbp) : null;
			}
		});
		editor.element.addEventListener('tokenized', () => {
			if (markers) markers.forEach(e => e ? e.destroy() : null);
			markers = cfg.styleGroupMark.markerAllow === 'yes' ? MarkerUtil.decorate() : null;
		});
