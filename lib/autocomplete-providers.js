	...
	if (editors && editors.length > 0) {
		...
		words = editors.map(te => {
			var ten = te.getFileName(),
				setw = te.__bufferWords || Array.from(new Set(te.buffer.getText().split(new RegExp('[\\W]+')))).filter(w => w && w.length >= wl);
			setw.forEach(w => {
				if (!desc['#' + w]) desc['#' + w] = [];
				desc['#' + w].push(ten);
			});
			if (!setw.changer) setw.changer = te.buffer.onDidStopChanging(ec => te.__bufferWords = null);
...
class FileProvider {
	...
	findMatchingSuggestions(prefix, options) {
		...
		var sugg = f,
			e = new RegExp('[' + ldp.split('').join('][\\w|\\W]*[') + '][\\w|\\W]*'),
			words = !ldp ? sugg : sugg.filter((s) => s.toLowerCase().match(e)),
			msugg = words.map((w) => {
				return {
					text: w,
...
class BasicProvider {
	...
	findMatchingSuggestions(prefix) {
		var sugg = reservedWords,
			...
			e = new RegExp('[' + plc.split('').join('][\\w|\\W]*[') + '][\\w|\\W]*'),
		...
		if (main.cfg.autoCompleteSort === 'yes' && plc && plc.charAt(0).match(new RegExp('\\w'))) cwords.sort((e1, e2) => sorter(e1, e2, e, plc));
		return cwords;
	}
}
...
class AsyncProvider {
	...
	filterData(prefix, sugg) {
		var plc = prefix.toLowerCase(),
			e = new RegExp('[' + plc.split('').join('][\\w|\\W]*[') + '][\\w|\\W]*'),
			words = !prefix ? sugg : sugg.filter(s => ((main.cfg.autoCompleteNS === 'yes' ? s.leftLabelHTML + '.' : '') + s.text).toLowerCase().match(e));
		if (main.cfg.autoCompleteSort === 'yes' && plc && plc.charAt(0).match(new RegExp('\\w'))) words.sort((e1, e2) => sorter(e1, e2, e, plc));
		return words;
	}
...
module.exports.word = new WordProvider();