...
var parsePropertyType = utils.parsePropertyType,
	...
	defDescriptor = utils.defDescriptor,
	defProtodox = utils.defProtodox,
...
function getDescInheritance(descjson, buffer) {
	readers.readInheritance(descjson, function(dj) {
		if (cfg.console === 'yes') console.log(dj, buffer);
	...
	if (cfg.descjson) {
		...
		if (!cfg.descjson.includes) {
			...
			rd.push({
				...
				description: 'Loading inherited classes JSON descriptors.\nTry again in few milliseconds.',
				rightLabelHTML: 'Inheritance',
	...
	static resolve(options) {
		...
		try {
			...
			if (ct) {
				...
				try {
					f = (cfg.editor.getPath() || "").split(path.sep);
					f.pop();
					f = f.join(path.sep);
					rp = path.resolve(f, v);
					if (fs.existsSync(rp)) {
